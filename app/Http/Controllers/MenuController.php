...
class MenuController extends Controller
{
    ...
    public function menudisplay()
    {
        ...
        return view('Admin.Menu.menu_management')->with('key', $data)->with('key1', $new);
    }

    public function updatedMenu(Request $request)
    {
        $menu = $request->menu;
        $array_menu = json_decode($menu, true);
        DB::table('menus')->truncate();
        $this->addOrUpdateMenu($array_menu);
        return redirect()->back()->with('success', 'successfully updated');
    }

    public function addOrUpdateMenu($menu, $parent = 0)
    {
        if (!empty($menu)) {
            foreach ($menu as $value) {
                ...
                $custom_menu_id = (empty($value['custom_menu_id']) || !isset($value['custom_menu_id'])) ? $value['url'] : $value['custom_menu_id'];

                DB::insert('insert into menus (menu_title, url, p_menu, icons) values (?, ?, ?, ?)', [$label, $url, $parent, $custom_menu_id]);

                $id =  DB::getPdo()->lastInsertId();
                if (array_key_exists('children', $value)) {
                    $this->addOrUpdateMenu($value['children'], $id);
                }
            }
        }
    }
